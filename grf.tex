...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\frame{\frametitle{Theory:\\How to generate stationary GRFs}
\begin{block}{Theorem 11 (Spectral method, continuous FT)}
...
What happens/changes if we want to apply the spectral method to generate GRF realizations on a grid in a bounded domain numerically, using the discrete Fourier transform (DFT)?\\
\medskip
\begin{enumerate}
...
\item Multiply by $\Lambda(\vec{k})$ which was calculated analytically via continuous FT on $\mathbb{R}^d$ (!)
\item Apply inverse DFT + normalization
\end{enumerate}
\textcolor{red}{$\Longrightarrow$ Conceptually, we start in Fourier space and impose the correlation $\hat{\chi}$ there! What kind of correlation do we obtain in real space with this method?}
}
